...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function tryLoadingImage(imgElement, teacherName, displayName) {
        secureLog(`[tryLoadingImage] Called for displayName: "${displayName}", teacherName variable for path: "${teacherName}"`);
        ...
        function tryNextImage() {
            if (currentAttempt >= patternAttempts.length) {
                secureWarn(`[tryLoadingImage] All patterns failed for "${displayName}". Using placeholder SVG.`);
                const imageNotAvailableText = (window.translations && window.translations[currentLang] && 
                    window.translations[currentLang].imageNotAvailable) ? window.translations[currentLang].imageNotAvailable : 'Immagine non disponibile';
            ...
            secureLog(`[tryLoadingImage] Attempt ${currentAttempt}/${patternAttempts.length} (${attemptType}) for "${displayName}": Trying URL: ${nextUrl}`);
            
            const img = new Image();
            img.onload = () => {
                secureLog(`[tryLoadingImage] SUCCESS: Loaded image for "${displayName}" from (${attemptType}) ${nextUrl}`);
                imgElement.src = nextUrl;
            };
            img.onerror = () => {
                secureWarn(`[tryLoadingImage] FAILED: Could not load image for "${displayName}" from (${attemptType}) ${nextUrl}.`);
    ...
    function showModal(clickEvent) {
        ...
        if (clickEvent) {
            try {
                ...
                secureError("Error setting transform origin:", error);
